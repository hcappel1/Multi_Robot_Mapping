...
public:
	...
	PassDown(geometry_msgs::PoseArray pass_down_frontier_send, geometry_msgs::PoseArray pass_down_path_send, vector<uint8_t> chosen_queue_send, int robots_remaining_send, bool backup_send) : pass_down_r4("R4_main_action_server", true)
	{
	...
	void SendActionRequest(){
		...
		pass_down_r4.sendGoal(msg, boost::bind(&PassDown::doneCb, this, _1, _2), boost::bind(&PassDown::activeCb, this));
	}

	void doneCb(const actionlib::SimpleClientGoalState& state,
				const r1_main_pkg::PassDownResultConstPtr& result)
	{
		ROS_INFO("[Robot 4 has completed]");
	}

	void activeCb()
	{
		ROS_INFO("[Information has been passed to robot 4]");
	}
};
...
public:
	...
	MoveBaseClient move_base_r3;
	geometry_msgs::PoseStamped frontier_pt;
	bool nav_completion;

	MoveBase(geometry_msgs::PoseStamped frontier_pt_) : move_base_r3("tb3_2/move_base", true)
	{
		ROS_INFO("[MoveBase object constructed]");
		MoveBaseClient move_base_r3("tb3_2/move_base", true);
		frontier_pt = frontier_pt_;
		SendGoal();
	}

	void SendGoal(){
		move_base_r3.waitForServer();

		move_base_msgs::MoveBaseGoal nav_goal;
		nav_goal.target_pose = frontier_pt;
		nav_goal.target_pose.pose.orientation.w = 1.0;

		move_base_r3.sendGoal(nav_goal, boost::bind(&MoveBase::doneCb, this, _1, _2), boost::bind(&MoveBase::activeCb, this), boost::bind(&MoveBase::feedbackCb, this, _1));
	}

	void doneCb(const actionlib::SimpleClientGoalState& state,
				const move_base_msgs::MoveBaseResultConstPtr& result)
	{
		ROS_INFO("[Robot 3 has arrived at its goal]");
		nav_completion = true;
	}
...
public:
	...
	{
		as_.start();
		choice_srv = nh_.serviceClient<frontier_pkg_tb::ChoiceMsg>("/choose_frontier_r3");
		choice_pub = nh_.advertise<geometry_msgs::PoseStamped>("/r3_chosen_pt", 1000);
		ROS_INFO("[R3 main action server active]");
	}
	...
	void executeCB(const r1_main_pkg::PassDownGoalConstPtr &msg){
		ros::Rate(1);
		bool success;
		ROS_INFO("[Robot 3 executing decision process]");
		...
		if (choice_srv.call(choice_msg)){
			ROS_INFO("[Robot 3 sent request for frontier choice]");
			...
			ROS_ERROR("[Could not send frontier request for robot 3]");
		}
		...
		while (ros::ok()){
			...
			actionlib::SimpleClientGoalState move_base_state = move_base.move_base_r3.getState();
			// actionlib::SimpleClientGoalState pass_down_state = pass_down.pass_down_r4.getState();
...
{
	ros::init(argc, argv, "R3_main_node");

	MainAction main_action("R3_main_action_server");
	ros::spin();
